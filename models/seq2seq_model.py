...
class Seq2SeqAttModel(BaseModel):
    def __init__(self, config, vocab, logger, trainable=True):
        super(Seq2SeqAttModel, self).__init__(config, vocab, logger)
        self._trainable = trainable
        self.seq2seqAttGraph = tf.Graph()
        # self.global_step = tf.Variable(0, trainable=False)

    def build_traineval(self):
        with self.seq2seqAttGraph.as_default():
            self.global_step = tf.Variable(0, trainable=False)
    ...
    def _setup_model(self):
        ...
        self.Encoder = encoder.Encoder(self._config, self._trainable)
        self.Decoder = decoder.DecoderAtt(self._config, self._vocab)
        self.logger.info('-model build done .')

    def _placeholder(self):
        # self.lr = tf.placeholder(name='leraning_rate', shape=(), dtype=tf.float32)
        self.img = tf.placeholder(
            shape=(None, None, None, 1),
            dtype=tf.int8, name='input_img')
        self.seqs = tf.placeholder(shape=(None, None), dtype=tf.int32, name='label')
        self.formula_length = tf.placeholder(shape=(None,), dtype=tf.int32, name='label_length')
        self.input_sequence = self.seqs[:, :-1]
        self.target_sequence = self.seqs[:, 1:]
    ...
    def init_session(self):
        ...
        if self._config.model.gpu_flage:
            ...
            _DeviceConfig = tf.ConfigProto(device_count={"CPU": 6, "GPU": 0},
                                           intra_op_parallelism_threads=0,
                                           inter_op_parallelism_threads=0)
        self.sess = tf.Session(config=_DeviceConfig, graph=self.seq2seqAttGraph)
    ...
    def _pipline(self):
        _EncoderOut = self.Encoder(self.img, self._config.model.encoder_type)
        self.logits_train, self.logits_valid = self.Decoder(
            _EncoderOut, self.droupout, self.input_sequence)

    def _cal_loss(self):
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=self.logits_train, labels=self.target_sequence)
        ...
        self.n_word = tf.reduce_sum(self.formula_length)
