...
def prenet(inputs, drop_rate, layer_sizes=[256, 128], scope=None, is_training=True):
    x = inputs
    with tf.variable_scope(scope or 'prenet'):
        for i, size in enumerate(layer_sizes):
            dense = tf.layers.dense(x, units=size, activation=tf.nn.relu, name='dense_%d' % (i+1))
            # x = tf.layers.dropout(dense, rate=drop_rate, name='dropout_%d' % (i + 1))\
            x = tf.nn.dropout(dense, keep_prob=drop_rate, name='dropout_%d' % (i + 1))
    return x
    ...
    with tf.variable_scope(scope):
        with tf.variable_scope('conv_bank'):
            ...
            conv_outputs = tf.concat(
                [conv1d(inputs, k, 128, tf.nn.relu, is_training, 'conv1d_%d' % k)
